    ...
    <li v-for="todo in todoListStore.sortedTodoList" :key="todo.flag" class="todo">
      <div class="todo-row">
        ...
        <div class="buttons-wrapper">
          <button class="delete-button" @click="todoListStore.deleteTodo(todo)">
            <svg
          ...
          <input v-model="todo.done" type="checkbox" name="todoCheckbox" class="todo-checkbox" />
        </div>
      </div>
      <div class="todo-row">
        <span v-if="translateFlag(todo.flag)" class="flag">
          <svg
            ...
            viewBox="0 0 24 24"
            fill="none"
            :class="'flag-' + translateFlag(todo.flag).toLowerCase()"
          >
            <path
